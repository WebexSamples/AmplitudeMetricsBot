    ...
    if 'files' in message_info:
        if 'text' in message_info:
            if message_info['text'] in bot.hears_file_to_function:
                ...
                # Edit this thingy to send data
                bot.hears_file_to_function[message_text](files=message_info['files'], room_id=message_info['roomId'],message = message_info)
                return "Works"
            
            elif '*' in bot.hears_file_to_function:
                bot.hears_file_to_function['*'](files=message_info['files'], room_id=message_info['roomId'],message= message_info)
            ...
            bot.default_attachment(files=message_info['files'], room_id=message_info['roomId'], message= message_info)
            return "Works"
        ...
        message_text = message_info[ "text" ]
        bot.hears_to_function[ message_text ]( room_id=message_info["roomId"], message= message_info)

        
    elif message_info["text"].strip() != "" and  message_info[ "text" ] not in bot.hears_to_function:
        bot.hears_to_function[ "*" ]( room_id=message_info["roomId"], message= message_info)
